...
/** Честно украдено из android.opengl.Matrix чтобы не елозить свои методы для работы
 * с матрицами.
 * Нативные методы переписаны на ненативные
 */
public class Matrix {
    ...
    public static void multiplyMM(float[] result, int resultOffset, float[] lhs, int lhsOffset, float[] rhs, int rhsOffset){
        for (int j=0; j<4; j++){                // цикл по столбцам
            for (int i=0; i<4; i++){            // цикл по строкам
                // умножаем i-тую строку первой матрицы на j-тый столбец второй
                result[j*4+i+resultOffset] =    lhs[i+0+lhsOffset]  *  rhs[4*j+0+rhsOffset] +
                                                lhs[i+4+lhsOffset]  *  rhs[4*j+1+rhsOffset] +
                                                lhs[i+8+lhsOffset]  *  rhs[4*j+2+rhsOffset] +
                                                lhs[i+12+lhsOffset] *  rhs[4*j+3+rhsOffset];
            }
        }
    }
    ...
    public static void multiplyMV(float[] resultVec, float[] lhsMat, float[] rhsVec){
        resultVec[0] = lhsMat[0]*rhsVec[0]+lhsMat[4]*rhsVec[1]+lhsMat[8]*rhsVec[2]+lhsMat[12]*rhsVec[3];
        resultVec[1] = lhsMat[1]*rhsVec[0]+lhsMat[5]*rhsVec[1]+lhsMat[9]*rhsVec[2]+lhsMat[13]*rhsVec[3];
        resultVec[2] = lhsMat[2]*rhsVec[0]+lhsMat[6]*rhsVec[1]+lhsMat[10]*rhsVec[2]+lhsMat[14]*rhsVec[3];
        resultVec[3] = lhsMat[3]*rhsVec[0]+lhsMat[7]*rhsVec[1]+lhsMat[11]*rhsVec[2]+lhsMat[15]*rhsVec[3];
    }
     ...
     * @param aspect virtualWidth to height aspect ratio of the viewport
     * @param zNear
     * @param zFar
     */
